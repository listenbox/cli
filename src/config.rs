...
impl Config {
    ...
    fn load_in(explicit: Option<&Path>, directory: PathBuf) -> Result<Self> {
        ...
        let mut values = match fs::read(path) {
            Ok(raw) => serde_yaml::from_slice::<CLIConfig>(&raw)
                .with_context(|| format!("decode CLI config {}", path.display()))?,
            ...
            Err(error) => {
                return Err(error).with_context(|| format!("load CLI config {}", path.display()));
            }
        };
